...
collapse.forEach((i) => {collapsesArr.push(new ItcCollapse(i))});
document.querySelectorAll(".faq__header").forEach((i) => {
	return i.addEventListener('click', (e) => {
		collapsesArr.forEach((i) => {i.hide()});
		let index = Array.prototype.indexOf.call(collapse, e.target.nextElementSibling);
		collapsesArr[index].toggle();
	})
});